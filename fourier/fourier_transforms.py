...
def DFT_1d_v1(arr: NDArray, inverse=False) -> NDArray:
    ...
    w = np.exp((w_exp_sign * 2.0j * np.pi) / n)
...
def DFT_1d_v2(arr: NDArray, inverse=False) -> NDArray:
    ...
    w_exp_sign = 1 if inverse else -1
    exponents = np.outer(np.arange(n), np.arange(n)) * w_exp_sign * (2.0j * np.pi / n)
...
def test_DFT_func_2(DFT_1d):
    ...
    x = np.array([1.0, 2 - 1j, -1j, -1 + 2j]).T
    y_DFT_actual = DFT_1d(x)
    y_DFT_pred = np.array([2.0, -2 - 2j, -2j, 4 + 4j]).T
    np.testing.assert_allclose(
        y_DFT_actual, y_DFT_pred, atol=1e-10, err_msg="DFT failed"
    )
