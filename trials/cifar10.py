...
from arena.resnets.resnet import ResNet34
...
def train(
    trainset, testset, epochs: int, loss_fn: Callable, batch_size: int, lr: float
) -> tuple[list, list]:
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch {epoch+1}/{epochs}, train loss is {loss:.6f}, accuracy is {accuracy}/{total}"
        )

    filename = "./resnets_resnet.pt"
