...
import convnets_solutions
import resnets_solutions
...
def conv_transpose1d_minimal(x: t.Tensor, weights: t.Tensor) -> t.Tensor:
    ...
    x_mod = convnets_solutions.pad1d(
        x, left=kernel_width - 1, right=kernel_width - 1, pad_value=0
    )
    weights_mod = rearrange(weights.flip(-1), "i o w -> o i w")

    return convnets_solutions.conv1d_minimal(x_mod, weights_mod)
...
def conv_transpose1d(x, weights, stride: int = 1, padding: int = 0) -> t.Tensor:
    ...
    x_mod = convnets_solutions.pad1d(
        x_spaced_out, left=padding_amount, right=padding_amount, pad_value=0
    )
...
def conv_transpose2d(
    ...
    x_mod = convnets_solutions.pad2d(
        x_spaced_out,
        left=pad_w_actual,
        right=pad_w_actual,
        top=pad_h_actual,
    ...
    return convnets_solutions.conv2d_minimal(x_mod, weights_mod)
...
class Generator(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.project_and_reshape = Sequential(
            OrderedDict(
                [
                    (
                        "fc",
                        convnets_solutions.Linear(
                            latent_dim_size, first_size, bias=False
                        ),
                    ),
                    (
                        "rearrange",
                        Rearrange(
                            "b (ic h w) -> b ic h w", h=first_height, w=first_height
                        ),
                    ),
                    ("bn", resnets_solutions.BatchNorm2d(generator_num_features)),
                    ("activation_fn", convnets_solutions.ReLU()),
                ]
            )
        )
        ...
        in_channels_list = (
            (generator_num_features / 2 ** t.arange(n_layers)).to(int).tolist()
        )
        out_channels_list = in_channels_list[1:] + [
            self.img_channels,
        ]
        ...
        for i, (ci, co) in enumerate(zip(in_channels_list, out_channels_list)):
            conv_layer = [
                ...
                (
                    "activation_fn",
                    convnets_solutions.ReLU() if i < n_layers - 1 else Tanh(),
                ),
            ]
            if i < n_layers - 1:
                conv_layer.insert(1, ("bn", resnets_solutions.BatchNorm2d(co)))
...
class Discriminator(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # note that the last out_channels is 3, for the colors of an RGB image
        out_channels_list = (
            (generator_num_features / 2 ** t.arange(n_layers)).to(int).tolist()[::-1]
        )
        in_channels_list = [
            self.img_channels,
        ] + out_channels_list
        ...
        for i, (ci, co) in enumerate(zip(in_channels_list, out_channels_list)):
            conv_layer = [
                ("conv", convnets_solutions.Conv2d(ci, co, 4, 2, 1)),
        ...
        self.classifier = Sequential(
            OrderedDict(
                [
                    ...
                    ("fc", convnets_solutions.Linear(final_size, 1, bias=False)),
                    ("sigmoid", Sigmoid()),
                ]
            )
        )
...
def train_generator_discriminator(
    ...
    t0 = time.time()
    if max_epoch_duration is None:
        max_epoch_duration = t.inf
    if print_netG_output_interval is None:
        print_netG_output_interval = t.inf
    last_interval = 0
    ...
    for epoch in range(epochs):
        ...
        for img_real, label in progress_bar:
            ...
            progress_bar.set_description(
                f"epoch={epoch}, steps={n_steps}, lossD={lossD.item():.4f}, lossG={lossG.item():.4f}"
            )
            ...
            if time.time() - t0 > print_netG_output_interval * (last_interval + 1):
                last_interval += 1
                utils.display_generator_output(
                    netG, netG.latent_dim_size, rows=2, cols=6
                )
