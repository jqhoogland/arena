...
from arena.convnets.module import Conv2d, Flatten, Linear, MaxPool2d, ReLU
from arena.resnets import utils
...
class BatchNorm2d(nn.Module):
    ...
    def forward(self, x: t.Tensor) -> t.Tensor:
        ...
        if not self.training:
            x_hat = (x - self.running_mean.reshape((1, -1, 1, 1))) / t.sqrt(
            ...
            self.running_mean = (
                1 - self.momentum
            ) * self.running_mean + self.momentum * batch_mean[0, :, 0, 0]
            self.running_var = (
                1 - self.momentum
            ) * self.running_var + self.momentum * batch_var[0, :, 0, 0]

            x_hat = (x - batch_mean) / t.sqrt(batch_var + self.eps)
            self.num_batches_tracked += 1

        return x_hat * self.weight.as_strided(
            size=x_hat.shape, stride=(0, 1, 0, 0)
...
class ResidualBlock(nn.Module):
    def __init__(self, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.left = Sequential(
            ...
            ReLU(),
            Conv2d(
                out_feats, out_feats, kernel_size=3, stride=1, padding=1, bias=False
            ),
            BatchNorm2d(out_feats),
        )
        ...
        if first_stride > 1:
            self.right = Sequential(
                Conv2d(
                    in_feats, out_feats, kernel_size=1, stride=first_stride, bias=False
                ),
                BatchNorm2d(out_feats),
            )
        else:
    ...
    def __init__(self, n_blocks: int, in_feats: int, out_feats: int, first_stride=1):
        ...
        self.blocks = nn.ModuleList(
            [
                ...
                if i == 0
                else ResidualBlock(out_feats, out_feats)
...
pretrained_resnet = models.resnet34(
    weights=models.ResNet34_Weights.IMAGENET1K_V1, progress=False
)
...
for i in range(min(len(my_resnet_state), len(pretrained_resnet_state))):
    ...
    if my_resnet_state[i][1].shape != pretrained_resnet_state[i][1].shape:
        ...
        print(
            my_resnet_state[i][0],
            pretrained_resnet_state[i][0],
            my_resnet_state[i][1].shape,
        )
...
def copy_weights(
    myresnet: ResNet34, pretrained_resnet: torchvision.models.resnet.ResNet
) -> ResNet34:
...
IMAGE_FOLDER = Path("../resnets/resnet_inputs")
...
with open("../resnets/imagenet_labels.json") as f:
