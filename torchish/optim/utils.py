...
def plot_fn(
    ...
    fig = (
        ...
        .update_layout(height=700, width=1600, title_font_size=40)
...
def plot_optimization_sgd(
    opt_fn_with_sgd: Callable,
    fn: Callable,
    xy: t.Tensor,
    x_range=[-2, 2],
    y_range=[-1, 3],
...
def plot_optimization(
    ...
    for i, (color, optimizer) in enumerate(zip(px.colors.qualitative.Set1, optimizers)):
        xys = opt_fn(
            fn, xy.clone().detach().requires_grad_(True), *optimizer, n_iters
        ).numpy()
        x, y = xys.T
        z = fn(x, y)
        optimizer_active = optimizer[0](
            [xy.clone().detach().requires_grad_(True)], **optimizer[1]
        )
...
def plot_optimization_with_schedulers(
    opt_fn_with_scheduler: Callable,
    fn: Callable,
    xy: t.Tensor,
    optimizers: list,
    schedulers: list,
    ...
    for i, (color, optimizer, scheduler) in enumerate(
        zip(px.colors.qualitative.Set1, optimizers, schedulers)
    ):
        optimizer_active = optimizer[0](
            [xy.clone().detach().requires_grad_(True)], **optimizer[1]
        )
